...
Run: python seeder/upload_matches.py
...
def process_excel_file(conn, file_path: Path, twd_sku_col: str, comp_sku_col: str,
    ...
    has_match_score = "MATCH_SCORE" in df.columns
    has_rank = "RANK" in df.columns

    # Filter to top 5 ranks only if RANK column exists
    if has_rank:
        before_filter = len(df)
        df = df[df["RANK"] <= 5]
        print(f"  Filtered to RANK <= 5: {len(df)} rows (from {before_filter})")
    ...
    for idx, row in df.iterrows():
        ...
        # Get MATCH_SCORE value (default to 1.0 if column doesn't exist)
        confidence = 1.0
        if has_match_score and not pd.isna(row["MATCH_SCORE"]):
        ...
        if dry_run:
            print(f"    [DRY RUN] Would match TWD:{twd_sku} -> {competitor_id}:{comp_sku} (needs review, score={confidence})")
            successful += 1
            unverified_count += 1
        else:
            # Always insert as unverified (needs review in UI)
            if insert_match(conn, twd_product_id, comp_product_id, competitor_id, confidence=confidence, is_verified=False):
                successful += 1
                unverified_count += 1
            else:
                failed += 1
...
def main():
    ...
    if not excel_files:
        print("No Excel files found (looking for twd_*.xlsx)")
        return
