...
# On Windows, reconfigure stdout/stderr to UTF-8 so crawl4ai's progress chars
...
class ScrapingConfig:
    ...
    max_concurrent: int = 2  # Reduced from 3 to 2 to prevent browser overload
    delay_between_requests: float = 1.5  # Increased from 1.0 to 1.5 for better stability
    timeout: int = 30
    ...
    # HomePro-only proxy (to bypass Cloudflare Turnstile)
    # Format: {"server": "http://host:port", "username": "user", "password": "pass"}
    # Only applied to homepro.co.th URLs - all other retailers are unaffected
    homepro_proxy: Optional[dict] = None
...
class Crawl4AIWrapper:
    ...
    def __init__(self, config: ScrapingConfig = None):
        ...
        self._max_scrapes_before_restart = 10  # Restart browser every 10 scrapes
    ...
    async def _cleanup_browser(self):
        """Safely close and clear the crawler instance."""
        if self.crawler:
            try:
    ...
    def _is_browser_alive(self) -> bool:
        """Check if the browser instance is still valid."""
    ...
    async def restart(self):
        """Restart the crawler instance (close and reinitialize)."""
        logger.info("Restarting Crawl4AI crawler to free resources")
        await self._cleanup_browser()
    ...
    async def scrape_url(
        ...
        # Proactive browser restart every N scrapes to prevent memory buildup
        ...
        # Reinitialize if browser is not alive
        if not self._is_browser_alive():
            logger.debug("Browser not alive, reinitializing...")
            await self.initialize()

        result = ScrapingResult(url=url, success=False)
        max_browser_reinit = 2

        # GlobalHouse: only 1 attempt — if it times out, the scraper falls back to requests
        effective_retries = 1 if 'globalhouse.co.th' in url else self.config.retry_attempts
...
(async () => {
    ...
    async function clickReadMoreButtons() {
        ...
        // The tab is an h5 element: <h5 class="horizontalTab-tabListItem-eWl"><span class="horizontalTab-label-XGH">ข้อมูลจำเพาะ</span></h5>
        const boonthavornSpecs = Array.from(document.querySelectorAll('h5')).find(h5 =>
            h5.textContent && h5.textContent.includes('ข้อมูลจำเพาะ') && !h5.className.includes('Active')
        );

        if (boonthavornSpecs) {
            // Scroll into view
            boonthavornSpecs.scrollIntoView({ behavior: 'instant', block: 'center' });
            await new Promise(resolve => setTimeout(resolve, 800));

            // Try comprehensive event triggering including pointer events (for React 17+)
            const rect = boonthavornSpecs.getBoundingClientRect();
            const eventOptions = {
                view: window,
                bubbles: true,
                cancelable: true,
            ...
            boonthavornSpecs.dispatchEvent(new PointerEvent('pointerdown', eventOptions));
            await new Promise(resolve => setTimeout(resolve, 50));
            boonthavornSpecs.dispatchEvent(new MouseEvent('mousedown', eventOptions));
            await new Promise(resolve => setTimeout(resolve, 50));
            boonthavornSpecs.dispatchEvent(new PointerEvent('pointerup', eventOptions));
            await new Promise(resolve => setTimeout(resolve, 50));
            boonthavornSpecs.dispatchEvent(new MouseEvent('mouseup', eventOptions));
            await new Promise(resolve => setTimeout(resolve, 50));
            boonthavornSpecs.dispatchEvent(new MouseEvent('click', eventOptions));
            boonthavornSpecs.click();
            await new Promise(resolve => setTimeout(resolve, 300));

            // Also try span
            const specSpan = boonthavornSpecs.querySelector('span');
            if (specSpan) {
                specSpan.click();
            }
            ...
            // After clicking specifications tab, click ALL "ดูรายละเอียดเพิ่มเติม" buttons to expand hidden content
            const allButtons = document.querySelectorAll('button');
            for (const btn of allButtons) {
                if (btn && btn.textContent && btn.textContent.includes('ดูรายละเอียดเพิ่มเติม')) {
    ...
    // HardwareWarehouse specific - block popup overlays
    if (window.location.href.includes('hardwarehouse.co.th')) {
        console.log('HardwareWarehouse: current URL:', window.location.href);

        // Block popup overlays so they don't obscure content
        const style = document.createElement('style');
        style.textContent = `
            .mfp-bg, .mfp-wrap, .mfp-ready,
            .modal, .popup, [class*="modal"], [class*="popup"],
            [class*="notification"], [class*="permission"],
            .swal2-container, .sweet-alert, [role="dialog"],
            div[style*="z-index: 9999"], div[style*="z-index: 10000"] {
        ...
        console.log('HardwareWarehouse: Done waiting, current URL:', window.location.href);
    } else {
        // Wait for initial page load for other sites
        await new Promise(resolve => setTimeout(resolve, 1500));
...
(async () => {
    await new Promise(resolve => setTimeout(resolve, 1500));
                ...
                # Inject actual URL into HardwareWarehouse JS code (for redirect detection)
                js_scroll_code = js_scroll_code.replace('__HW_TARGET_URL__', url)

                # Perform the crawl using CrawlerRunConfig if available (v0.7.x+)
                if CrawlerRunConfig is not None and self.config.use_browser:
                    is_hardwarehouse = 'hardwarehouse.co.th' in url
                    is_homepro = 'homepro.co.th' in url
                    is_globalhouse = 'globalhouse.co.th' in url
                    # Per-retailer page timeouts
                    if is_hardwarehouse:
                        timeout_ms = 90000
                    elif is_homepro:
                        timeout_ms = 20000  # Hard cap: don't wait more than 20s for HomePro
                    elif is_globalhouse:
                        timeout_ms = 20000  # GlobalHouse: price API is fast; 20s is enough
                    else:
                        timeout_ms = 45000
                    # Delay before capturing HTML to ensure API responses are rendered
                    delay_before_html = 0.5 if is_hardwarehouse else 0.1

                    # For HomePro only: attach proxy config if provided (bypasses Cloudflare)
                    homepro_proxy_config = None
                    if is_homepro and self.config.homepro_proxy:
                        homepro_proxy_config = self.config.homepro_proxy
                    ...
                    run_config = CrawlerRunConfig(
                        ...
                        proxy_config=homepro_proxy_config,
                    )
                    # Wrap arun with asyncio timeout to catch frozen pages
                    # HomePro needs 120s (25s retry delay + page load); GlobalHouse/others need less
                    asyncio_timeout = 120 if is_homepro else 30
                    try:
                        ...
                        logger.error(f"Page load timeout for {url} - freezing detected")
                        result.error_message = "Page load timeout - likely frozen"
                        continue
                else:
                ...
                # Check if browser crashed - reinitialize with retry limit
                if self._is_browser_closed_error(e):
                    if self._browser_reinit_attempts < max_browser_reinit:
                        ...
                        logger.warning(
                            ...
                            f"(reinit {self._browser_reinit_attempts}/{max_browser_reinit})..."
                        )
                        try:
                            await self.initialize()
                            ...
                            try:
                                ...
                                logger.error(f"Text mode fallback also failed: {text_err}")
                    else:
                        logger.warning(f"Max browser reinit attempts ({max_browser_reinit}) reached")
